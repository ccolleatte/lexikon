<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			const response = await api.post<OAuthCallbackResponse>('/auth/oauth/google/callback', {
				code
			});
			...
			error = 'Failed to complete Google authentication. Please try again.';
			isLoading = false;
		}
	});
</script>
...
<svelte:head>
	<title>Authenticating with Google - Lexikon</title>
</svelte:head>
	...
	<div class="max-w-md w-full text-center space-y-6">
		{#if isLoading}
			...
			<div class="space-y-4">
				...
				<h2 class="text-2xl font-bold text-gray-900">Authenticating with Google...</h2>
